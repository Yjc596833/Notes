 ...
 * @LastEditTime: 2021-05-20 10:43:43
...
setrange:{setrange key offset value} 用指定的字符串覆盖给定 key 所储存的字符串值，覆盖的位置从偏移量 offset 开始
...
##### aof [append only file]
        ...
        aof运行效率要慢于rdb,同步策略效率较好，不同步效率和rdb相同
...
#### 事务 [ Redis 事务的执行并不是原子性的 ]
 ...
 ##### 悲观锁/乐观锁/CAS（check and set）
       ...
       悲观锁：备份数据的时候可以用，行锁、表锁 等。。。
       乐观锁：版本号等机制，乐观锁适用于多读的应用类型，这样可以提高吞吐量[提交版本必须大于记录当前版本才能执行更新]
...
######  监控 WATCH  /  UNWATCH
        ...
        eg:set balance 100;set debt 0;watch balance; multi; decrby balance 20; incrby debt 20; exec;

        
        [有加塞篡改，先监控在开启multi]

        在watch的时候，在另一个终端，watch的字段的值发生了改变，即被加塞的情况下，会执行失败[事务被打断]
        ...
        一旦执行了exec,之前加的监控锁都会被取消掉。Watch指令，类似乐观锁，事务提交时，如果key的值已别的客户端改变，
        比如某个list已被别的客户端push/pop过了，整个事务队列都不会被执行。
        通过watch命令在事务执行之前监控了多了keys,倘若在watch之后有任何key的值发生了变化，
        exec命令执行的事务都将被放弃，同时放回nullmulti-bulk应答已通知调用者事务执行失败
...
#### 发布和订阅机制
        ...
        2.消息发布：publish c2 hello
        3.订阅多个，通配符*， psubscribe new*


#### master/slaver,master以写为主，slaver以读为主
